            ...
            for c in stmts:
                if (c.__class__.__name__ == 'IfStatement'):
                    ...
                    CDFG.add_edge(p_node_name, c_node_name, cond=[], \
                      ...
                      c.__class__.__name__ == 'NonblockingSubstitution'):
                    CDFG.node[p_node_name]['statements'].append(c)
                    CDFG.node[p_node_name]['label'] = CDFG.node[p_node_name]['label'] + '\n' + \
                            generate_code(c)
...
def construct_fused_CDFG(CDFGS, Ports):
    ...
    for CDFG in CDFGS:
        ...
        root_node = get_root_node(CDFG)[0]
        cdfg_type = CDFG.node[root_node]['typ']
        ...
        if cdfg_type == 'Assign':
            ...
            for uvar in use_var:
                if uvar not in use_var_proc.keys():
                    use_var_proc[uvar] = [root_node]
                else:
                    use_var_proc[uvar].append(root_node)
            for dvar in def_var:
                if dvar not in def_var_proc.keys():
            ...
            leaf_node = get_leaf_nodes(CDFG)[0]
            use_var = CDFG.node[root_node]['use_var']
            def_var = CDFG.node[leaf_node]['def_var']
...
def get_var_use_chain(CDFGS, PathSets, Params):
    ...
    for idx1 in range(len(CDFGS)):
        ...
        cdfg_type = CDFG.node[get_root_node(CDFG)[0]]['typ']
        if cdfg_type == 'Assign':
            node_name = CDFG.nodes()[0]
            ast = CDFG.node[node_name]['ast']
            def_var = get_lvalue(ast.left)
            use_vars = []
            ...
            CDFG.node[node_name]['def_var'].append(def_var)
            CDFG.node[node_name]['use_var'].extend(use_vars)

        elif cdfg_type == 'Always':
            root_node = get_root_node(CDFG)[0]
            clocked = CDFG.node[root_node]['clk_sens']
            is_sens_empty = True if not CDFG.node[root_node]['sens'] else False
            for Path in PathSet:
                cond = []
                for idx2 in range(1, len(Path)):
                    ...
                    if statements:
                        for statement in statements:
                            ...
                            use_vars_ = list(set(use_vars + CDFG.node[root_node]['use_var']))
                            CDFG.node[root_node]['use_var'] = use_vars_
                            
                            # Verilog syntax sens_vars are the only ones that are on the rvalue
                            sens_vars = []
                            if not clocked and is_sens_empty:
                                sens_vars = list(set(use_vars + CDFG.node[root_node]['sens']))
                                CDFG.node[root_node]['sens'] = sens_vars
                ...
                for cond_ in cond:
                    if cond_[0]:
                        ...
                        # as accumulated from the different paths of an Always CDFG. Same variable
                        # can be used in different paths to define different varuiables.
                        use_vars = list(set(cond_[0] + CDFG.node[root_node]['use_var']))
                        CDFG.node[root_node]['use_var'] = use_vars
                        del use_vars
            ...
            for Path in PathSet:
                ...
                for idx2 in range(1, len(Path)):
                    ...
                    if statements:
                        ...
                        def_vars = list(set(def_vars + CDFG.node[leaf_node]['def_var']))
                        CDFG.node[leaf_node]['def_var'] = def_vars
...
def temporal_cone(var, temp_length, temp_length_max, vtarget, PageRank, dep_g, \
    ...
    if temp_length < temp_length_max:
        ...
        for v in V:
            ...
            #$
            Ir_vtarget = cone.node[gvn(var, 0)]['importance']
            ...
            # On my hunch
            #$
            Cr_v_target = Cr_v_target + cone.node[gvn(var, 0)]['complexity']
...
def process_targets(cones, top_module, ELABORATE_INFO, scope_module_map):
    ...
    for key in scope_module_map.keys():
        module_name = scope_module_map[key]
        for iport in ELABORATE_INFO[module_name]['ports']['IPort'].keys():
            signals[key + '.' + iport] = ELABORATE_INFO[module_name]['ports']['IPort'][iport]

        for oport in ELABORATE_INFO[module_name]['ports']['OPort'].keys():
            signals[key + '.' + oport] = ELABORATE_INFO[module_name]['ports']['OPort'][oport]

        for reg in ELABORATE_INFO[module_name]['ports']['Reg'].keys():
            signals[key + '.' + reg] = ELABORATE_INFO[module_name]['ports']['Reg'][reg]

        for wire in ELABORATE_INFO[module_name]['ports']['Wire'].keys():
...
def Linking(ELABORATE_INFO, top_module):
    ...
    while InstanceQueue:
        ...
        for inst in curr_modinsts.keys():
            ...
            for pname in portmap.keys():
                ...
                # NOTE: For CDFG
                if pname in p_output_to_be_connected.keys():
                    onodes = p_output_to_be_connected[pname]
                    node3 = ['.'.join(nxt_scope) + '.' + onode for onode in onodes]
                elif pname in p_input_to_be_connected.keys():
