...
def analyze(Assertion_Results, top_module, target, rank_assertion, cone, aggregate, engine, verilog_files, include_directory):
    ...
    if not aggregate:
        ...
        Assertion_Results = ODict(sorted(list(Assertion_Results.items()), key=lambda t: t[1]['IRank'], reverse=True))
        ...
        total = len(list(Assertion_Results.keys()))
        ...
        gfile.write('#' * (len(treport) + 2) + '\n')
        gfile.write('\n\n')
        Report = ODict(sorted(list(Report.items()), key=lambda t: t[1][0], reverse=True))
        ...
        assertion_index_list = list(Report.keys())
        irank_list = [Report[key][0] for key in list(Report.keys())]
        imp_list = [Report[key][1] for key in list(Report.keys())]
        complx_list = [Report[key][2] for key in list(Report.keys())]
        ...
        Assertion_Results = ODict(sorted(list(Assertion_Results.items()), key=lambda t: t[1]['ARank'], reverse=True))
        ...
        assertion_index_list = list(Report.keys())
        arank_list = [Report[key][0] for key in list(Report.keys())]
        irank_list = [Report[key][1] for key in list(Report.keys())]
        srank_list = [Report[key][2] for key in list(Report.keys())]
        imp_list = [Report[key][3] for key in list(Report.keys())]
        complx_list = [Report[key][4] for key in list(Report.keys())]
...
def importance(rank_assertion, cone):
    ...
    for key in list(rank_assertion.keys()):
        ...
        for var_val_pair in antecedent:
            ...
            if match_n:
                ...
                if tstamp:
                    ...
                    cone_node = node
                importance = cone.nodes[cone_node]['importance'] + importance
        ...
        for var_val_pair in antecedent:
            ...
            if match_n:
                ...
                if tstamp:
                    ...
                    cone_node = node
                complexity = cone.nodes[cone_node]['complexity'] + complexity
...
def analyze_for_rank(target, rank_assertion, cone, engine):
    ...
    for node in cone.nodes():
        imp = round(cone.nodes[node]['importance'], 5)
        complx = cone.nodes[node]['complexity']
        cfile.write(node + '\n')
        cfile.write('Importance: ' + str(imp) + '\n')
        cfile.write('Complexity: ' + str(complx) + '\n')
        cfile.write('Rank: ' + str(round(cone.nodes[node]['rank'], 5)) + '\n\n')
...
def analyze_for_aggregation(target, rank_assertion, cone, engine, verilog_files, include_directory, top_module, Assertion_Results):
    ...
    IRank = ODict(sorted(list(IRank.items()), key=lambda t: t[1], reverse=True))

    Ranking = {}
    Ranking['IRank'] = list(IRank.keys())
    Ranking['SRank'] = list(SRank.keys())
...
def kendall_tau_distance(KMList, Ranking):
    ...
    for Metric in Metrics:
        ...
        pairs = itools.combinations(list(range(0, len(KMList))), 2)
        for x, y in pairs:
            #a = RMList[x] - RMList[y]
            #b = KMList[x] - KMList[y]
