...
if __name__ == "__main__":
    ...
    CONFIG = set_config_file_path(CMD_LINE_OPTIONS['CFG'], CMD_LINE_OPTIONS['VCD'] != '', \
            list(CMD_LINE_OPTIONS['CLK'].keys())[0] == 'DEFAULT_CLOCK')
    # pp.pprint(CONFIG)
    ...
    for vfile in CMD_LINE_OPTIONS['VFILES']:
        ...
        if not list(ModuleDefs_.keys()):
            fatal_error('No module definition found in the source verilog file(s)')
        ModuleDefs = ModuleDefs + list(ModuleDefs_.keys())
    ...
    if CMD_LINE_OPTIONS['STATICGRAPH']:
        ...
        for t_module in list(ELABORATE_INFO.keys()):
        ...
        PageRankO = ODict(sorted(list(complete_pagerank.items()), key=lambda t: t[1], reverse=True))
        ...
        for key in list(complete_cones.keys()):
    ...
    targets = list(target_cones.keys())
