...
class signal(object):
    ...
    def print_signal(self):
        ...
        for cycle, value in iter(sorted(self.value_table.items())):
    ...
    def optimize_path(self):
        ...
        while "END EXECUTION" not in statement:
            line_w_variables = ""
            line = ""
            if (statement.isdigit() or statement in list(mc.OPERATORS.values()) or statement in list(mc.SINGLE_OPS.values()) or "PART" in statement):
                while (statement.isdigit() or statement in list(mc.OPERATORS.values()) or statement in list(mc.SINGLE_OPS.values()) or "PART" in statement):
                    line += statement.replace("PART","")
                    line_w_variables += str(var_statement)
                    statement = next(iterator)
                    var_statement = next(var_iterator)
            else:
                line = statement
    ...
    def replace_signals(self, cycle, value_table, nodes):
        ...
        for index in range(len(self.executable_path)):
            ...
            for signal, item in value_table.items():
    ...
    def run_executable(self, cycle):
        ...
        self.optimized_path = [_f for _f in self.optimized_path if _f] # fastest
...
class signal_tree(object):
    ...
    def randomize_all(self, cycle):
        for signal, graph in self.signals.items():
...
class NumericStringParser(object):
   ...
   ################################################################
    def eval(self, num_string, condition=False, parseAll=True):
        ...
        num_string = num_string.replace("~|", "@")
        num_string = num_string.replace("~", "(" + str(sys.maxsize) + "^")
        num_string = num_string.replace("&&", "&")
        num_string = num_string.replace("||", "&")
        first = True
        not_replace = set()
        for i in num_string.split("(" + str(sys.maxsize) + "^"):
            if (first):
                first = False
                continue
            for g in i:
        ...
        for i in list(not_replace):
            num_string = num_string.replace("(" + str(sys.maxsize) + "^" + i, "(" + str(sys.maxsize) + "~" + i + ")")
...
def generate_graph(consequent, full_signal_tree, directory_path, topmodule):
   ...
   if (not full_signal_tree.nodes):
      ...
      if not nodes:
         nodes = dict()
         for tk, tv in sorted(list(terms.items()), key=lambda x:str(x[0])):
...
def get_module(verilog_files, include_directory, topmodule):
    ...
    for key, item in modules["terms"].items():
    ...
    for key, item  in modules["binddict"].items():
        item = item[0]
...
def get_modules(ast, ModuleDefs, ModuleInstances):
    ...
    if (ast.__class__.__name__ == 'ModuleDef'):
        ...
        module = ast.module
        inst_name = ast.name
        if module not in list(ModuleInstances.keys()):
...
def getDefUseTarget(Ports):
    ...
    inputs = list(Ports['IPort'].keys())
    outputs = list(Ports['OPort'].keys())
    regs = list(Ports['Reg'].keys())
    wires = list(Ports['Wire'].keys())
...
def parse_ast_find_def(ast, def_vars, var_def_chain, cond_vars, predicates):
    ...
    #        parse_ast_find_def(c, def_vars, var_def_chain, cond_vars)
        ...
        if curr_def_var in def_vars and curr_def_var in list(var_def_chain.keys()):
        ...
        elif curr_def_var in def_vars and curr_def_var not in list(var_def_chain.keys()):
            if rhs_constant:
                conds = list(itertools.chain(*cond_vars))
...
#            parse_ast_find_use(c, use_vars, var_use_chain, predicates)
        ...
        for use_var in curr_use_vars:
            if use_var in use_vars and use_var in list(var_use_chain.keys()):
                var_use_chain[use_var].append(tuple([ast, typ, left_var, ast.lineno]))
            elif use_var in use_vars and use_var not in list(var_use_chain.keys()):
...
def get_case_predicates(caselist, comp_nodes, predicates):
    ...
    if key not in list(predicates.keys()):
        predicates[key] = []
...
def construct_var_dep_graph(var_use_chain, var_def_chain, use_vars):
    ...
    for indep_var in list(var_use_chain.keys()):
    ...
    for dep_var in list(var_def_chain.keys()):
...
def generate_coverage(verilog_files, include_directory, list_assertions, directory_path, topmodule):
    ...
    for key in assertion_dict:
        ...
        for key, value in assertion_value_table.items():
...
def run_coverage(xxx_todo_changeme,target, assertion_per_core, i):
...
def write_output_files(assertion, ranking, top, signal):
    ...
    with open("./goldmine.out/" + top + "/coverage/" + signal + "/output.cov", "w") as file:
        ...
        file.write("#############################\n\n")
        for x in sorted(list(ranking.items()), key=operator.itemgetter(1), reverse=True):
            assert_key = str(x[0])
            ...
            file.write("\n\n")
        temp = sorted(list(pretty_table.items()), key=operator.itemgetter(1), reverse=True)
        pretty_table = dict()
...
def run_threaded(xxx_todo_changeme1):
    (options,args) = xxx_todo_changeme1
    print_start()
